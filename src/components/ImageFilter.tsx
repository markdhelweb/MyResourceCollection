...
const ImageFilter = () => {
    ...
    const buttonCaptions = ['all', 'documentation', 'designtools', 'frontendtools', 'javascript Libraries', 'typography', 'color', 'icons',
                            'htmltemplates', 'codesnippets', 'githubtools', 'notion', 'tools online', 'learning online'];
            ...
            <div className="flex w-full md:justify-center items-start md:gap-6 gap-3 flex-wrap">
                {
                    buttonCaptions.map((filter) => (
                        <Button key={filter} onClick={() => handleFilterClick(filter)} type="button" className={`focus:outline-none border-2 border-blue-600 hover:bg-blue-700 font-medium rounded-lg text-sm px-5 text-white py-2.5 mb-2 capitalize ${activeFilter === filter ? "bg-blue-800" : " "}`}>
